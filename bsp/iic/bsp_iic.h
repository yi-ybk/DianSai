 ...
 * @file    bsp_iic.h
 * @brief   IIC底层驱动接口声明
 * @details 同时支持MSPM0硬件I2C和GPIO模拟软件IIC，向上层模块提供统一的设备地址访问接口。
 */
#pragma once
...
typedef enum
{
    IIC_BUS_HARDWARE = 0, // 使用MSPM0硬件I2C外设
    IIC_BUS_SOFTWARE,     // 使用GPIO模拟IIC时序
} IIC_Bus_Mode_e;
...
typedef struct
{
    ...
    uint32_t GPIO_Pin;   // GPIO_PIN_0,GPIO_PIN_1...
    uint32_t GPIO_IOMUX; // SysConfig 生成的 IOMUX_PINCMx 索引
} IIC_GPIO_Config_s;

/* 软件IIC初始化配置 */
typedef struct
{
    IIC_GPIO_Config_s scl; // SCL时钟线
    IIC_GPIO_Config_s sda; // SDA数据线
    uint32_t delay_us;     // 半个IIC时钟周期延时,为0时默认2us
} IIC_Soft_Config_s;

/* i2c实例 */
typedef struct iic_temp_s
{
    IIC_HardwareHandle_t *handle; // 硬件I2C句柄
    uint8_t dev_address;          // 7位从机地址

    IIC_Bus_Mode_e bus_mode;               // 总线类型
    IIC_Work_Mode_e work_mode;             // 工作模式
    IIC_Soft_Config_s soft_config;         // 软件IIC配置
...
typedef struct
{
    ...
    IIC_Work_Mode_e work_mode;       // 工作模式
    IIC_Soft_Config_s soft_config;   // 软件IIC GPIO配置
 ...
 *
 */
uint8_t IICTransmit(IICInstance *iic, const uint8_t *data, uint16_t size, IIC_Seq_Mode_e mode);
