...
bool OledInit(Oled_t *oled, const OledInitConfig_t *config)
{
    ...
    if ((!OledWriteCommand(oled, 0xAEU)) ||
        ...
        (!OledWriteCommand(oled, 0x40U)) ||
        (!OledWriteCommand(oled, 0x81U)) ||
        (!OledWriteCommand(oled, 0x7FU)) ||
        (!OledWriteCommand(oled, oled->init_config.rotate_180 ? 0xA0U : 0xA1U)) ||
        (!OledWriteCommand(oled, oled->init_config.inverted ? 0xA7U : 0xA6U)) ||
        (!OledWriteCommand(oled, 0xA8U)) ||
        (!OledWriteCommand(oled, (uint8_t)(height - 1U))) ||
        (!OledWriteCommand(oled, 0xA4U)) ||
        (!OledWriteCommand(oled, 0xD3U)) ||
        (!OledWriteCommand(oled, 0x00U)) ||
        (!OledWriteCommand(oled, 0xD5U)) ||
        (!OledWriteCommand(oled, 0x80U)) ||
        (!OledWriteCommand(oled, 0xD9U)) ||
        (!OledWriteCommand(oled, 0xF1U)) ||
        (!OledWriteCommand(oled, 0xDAU)) ||
        (!OledWriteCommand(oled, (height == 64U) ? 0x12U : 0x02U)) ||
        (!OledWriteCommand(oled, 0xDBU)) ||
        (!OledWriteCommand(oled, 0x40U)) ||
        (!OledWriteCommand(oled, 0x8DU)) ||
        (!OledWriteCommand(oled, 0x14U)))
    {
        return false;
    }
...
static bool OledWriteCommand(Oled_t *oled, uint8_t command)
{
    ...
    buffer[1] = command;
    return IICTransmit(oled->iic, buffer, sizeof(buffer), IIC_SEQ_RELEASE) != 0U;
}
