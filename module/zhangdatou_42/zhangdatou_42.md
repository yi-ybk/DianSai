...
STM32 与电机之间必须使用 CAN 收发器，CANH/CANL 总线两端各接 `120Ω` 终端电阻并共地。

当前工程 CAN1 使用 42 MHz 时钟和以下参数产生 500 kbit/s：
...
`MX_CAN1_Init()` 应在创建电机对象前完成。对象可定义在 `application/robot.c`：
...
static const Zdt42InitConfig_t stepper_config =
    ZDT42_INIT_CONFIG_DEFAULT(&hcan1, 1U);
