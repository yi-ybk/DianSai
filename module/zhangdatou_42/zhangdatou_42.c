...
bool Zdt42Init(Zdt42_t *motor, const Zdt42InitConfig_t *config)
{
    ...
    if ((motor == NULL) || (config == NULL) ||
        (config->can_handle == NULL) || (config->motor_id == 0U))
        return false;
    ...
    can_config.can_handle = motor->init_config.can_handle;
    ...
    can_config.can_module_callback = Zdt42CanCallback;
    can_config.id = motor;
...
static void Zdt42CanCallback(CANInstance *instance)
{
    ...
    if (instance->rx_buff[length - 1U] != motor->init_config.checksum)
    {
        motor->data.checksum_error_count++;
        return;
    }

    memcpy(motor->data.raw_data, instance->rx_buff, length);
    motor->data.raw_length = length;
    motor->data.last_function = instance->rx_buff[0];
    motor->data.last_response = length > 2U ? instance->rx_buff[1] : 0U;
    motor->data.last_packet_index = (uint8_t)(instance->rx_message_id & 0xFFU);
    if (length == 3U)
    {
        motor->data.command_received = instance->rx_buff[1] == 0x02U;
        motor->data.motion_complete = instance->rx_buff[1] == 0x9FU;
    }

    if ((instance->rx_buff[0] == ZDT42_PARAM_SPEED) && (length == 5U))
    {
        uint16_t speed = ((uint16_t)instance->rx_buff[2] << 8U) |
                         (uint16_t)instance->rx_buff[3];
        motor->data.speed_rpm = instance->rx_buff[1] != 0U ? -(float)speed : (float)speed;
    }
    else if ((instance->rx_buff[0] == ZDT42_PARAM_POSITION) && (length == 7U))
    {
        uint32_t position = ((uint32_t)instance->rx_buff[2] << 24U) |
                            ((uint32_t)instance->rx_buff[3] << 16U) |
                            ((uint32_t)instance->rx_buff[4] << 8U) |
                            (uint32_t)instance->rx_buff[5];
        motor->data.position_deg = (float)position * 360.0f / ZDT42_POSITION_COUNTS_PER_REV;
        if (instance->rx_buff[1] != 0U)
            motor->data.position_deg = -motor->data.position_deg;
    }

    motor->data.rx_frame_count++;
    motor->data.last_rx_tick = HAL_GetTick();
