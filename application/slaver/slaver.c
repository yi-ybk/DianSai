...
bool SlaverInit(Slaver_t *slaver, const SlaverInitConfig_t *config)
{
    ...
    setup_ok = (slaver->usart != NULL) &&
               SlaverRegisterInstance(slaver) &&
               USARTConfigureDMA(slaver->usart, &slaver->init_config.dma_config) &&
               USARTStartReceiveDMA(slaver->usart);
    if (setup_ok)
    ...
    NVIC_ClearPendingIRQ(slaver->init_config.uart_irqn);
    NVIC_SetPriority(slaver->init_config.uart_irqn,
                     slaver->init_config.uart_irq_priority);
    NVIC_EnableIRQ(slaver->init_config.uart_irqn);
    return true;
}
...
static bool SlaverConfigIsValid(const SlaverInitConfig_t *config)
{
    if ((config == NULL) || (config->uart_handle == NULL) ||
        ...
        (config->dma_config.dma == NULL) ||
        (config->dma_config.rx_channel == USART_DMA_CHANNEL_INVALID) ||
